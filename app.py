...
def plot_municipality_lines(df_long, title, height=350, metric_label="값", show_avg=True, expected_munis=None):
    ...
    # 호버박스가 잘리지 않게 함
    _hover_font = 13 if _n_legend <= 14 else (12 if _n_legend <= 22 else 11)
    # 호버박스가 화면 아무 위치에서나(맨 아래쪽에서 hover해도) 안 잘리려면, 커서
    # 위치에 따라 위/아래로 확보 가능한 공간이 절반씩일 수 있다고 보고 넉넉히 2배로 확보
    # (줄 사이 여백은 최대한 촘촘하게: 글씨 크기 + 1 정도로만 잡아서 글씨는 키우되 간격은 좁게)
    _hover_room = _n_legend * (_hover_font + 1) * 2
    if _n_legend > 14:
